...
class UTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        for i in range(len(encoder_out['encoder_out'])):
            ...
            weight_encoder_out = []
            for j in range(len(encoder_out['encoder_out'][i:])):
                weight_encoder_out.append(math.exp(len(encoder_out['encoder_out'])-1-j) / sum_exp * encoder_out['encoder_out'][i])
            encoder_out_list.append(sum(weight_encoder_out))'''
...
class UTransformerEncoderLayer(nn.Module):
    """Encoder layer block.
...
class UTransformerDecoderLayer(nn.Module):
    """Decoder layer block.
...
@register_model_architecture('utransformer', 'utransformer')
def ubase_architecture(args):
...
@register_model_architecture('utransformer', 'utransformer_iwslt_de_en')
